...
def identity_name(json_data):
    ...
    @param json_data: JSON data of the bio.tools tool.
    @type json_data: dict
    @return: Name of the bio.tools tool.
    @rtype: str
    """
...
def identity_name_insensitive(json_data):
    ...
    @param json_data: JSON data of the bio.tools tool.
    @type json_data: dict
    @return: Name of the bio.tools tool, lowercased for case-insensitive comparison.
    @rtype: str
    """
...
def identity_homepage(json_data):
    ...
    @param json_data: JSON data of the bio.tools tool.
    @type json_data: dict
    @return: Homepage of the bio.tools tool.
    @rtype: str
    """
...
def identity_biotoolsID_unprefixed(json_data):
    """
    Return the biotoolsID of the bio.tools tool from JSON, ignoring 'systematic' prefixes.
    For now systematics prefixes are 'bioconductor-' only.
    @param json_data: JSON data of the bio.tools tool.
    @type json_data: dict
    @return: biotoolsID of the bio.tools tool without 'bioconductor-' prefix.
    @rtype: str
    """
...
def identity_doi(json_data):
    ...
    @param json_data: JSON data of the bio.tools tool.
    @type json_data: dict
    @return: Set of publication dois of the bio.tools tool.
    @rtype: set
    """
    dois = set()
...
def create_match_upsetplot(data, filename, pattern):
    ...
    @param data: Dictionary of match data.
    @type data: dict
    @param filename: Path to save the UpSet plot.
    @type filename: str
    @param pattern: File pattern for the JSON files (used for plot title).
    @type pattern: str
    """
...
def compare_files(pattern1, pattern2, identity_methods, upset1_path, upset2_path):
    ...
    @param pattern1: File pattern for the first set of JSON files.
    @param pattern2: File pattern for the second set of JSON files.
    @param identity_methods: List of identity methods to use for comparison.
    @param upset1_path: Path to save the Upset plot summarising matches in the first set.
    @param upset2_path: Path to save the Upset plot summarising matches in the second set.
    """
    ...
    for file1, json1 in tqdm(
        json_data1.items(), desc="Processing dataset", unit="file"
    ):
        if not json1:
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare two sets of JSON files based on configurable identity functions."
    )
    parser.add_argument(
        "pattern1", help="File pattern for the first set of JSON files."
    )
    parser.add_argument(
        "pattern2", help="File pattern for the second set of JSON files."
    )
    parser.add_argument(
        "--results",
        help="Path to save the comparison results JSON file.",
        default=tempfile.NamedTemporaryFile(delete=False, suffix=".json").name,
    )
    parser.add_argument(
        "--upset1",
        help="Path to save the Upset plot summarising matches in the first set.",
        default=tempfile.NamedTemporaryFile(delete=False, suffix=".png").name,
    )
    parser.add_argument(
        "--upset2",
        help="Path to save the Upset plot summarising matches in the second set.",
        default=tempfile.NamedTemporaryFile(delete=False, suffix=".png").name,
    )
    parser.add_argument(
        "--methods",
        nargs="*",
        choices=IDENTITY_FUNCTIONS.keys(),
        default=list(IDENTITY_FUNCTIONS.keys()),
    ...
    comparison_result = compare_files(
        args.pattern1, args.pattern2, args.methods, args.upset1, args.upset2
    )

    with open(args.results, "w", encoding="utf-8") as outfile:
        json.dump(comparison_result, outfile, indent=4)
        logging.info(
            f"Matching results for {args.pattern1} against {args.pattern2} saved to {args.results}"
        )
