...
def enrich_dois(path):
    ...
    files = [
        "bioconda_" + dirname + file_types["yaml"],
        dirname + file_types["json"],
    ...
    def extract_doi_from_json(parsed_json):
        ...
        if "publication" in parsed_json:
            ...
            for publication in publications:
                if (
                    "type" in publication
                    and "doi" in publication
                    and publication["type"] == "Primary"
                ):
    ...
    def get_doi_identifiers_from_yaml(parsed_yaml, isDebian=False):
        if isDebian:
            ...
            print(
                [
                    item["value"]
                    for item in parsed_yaml.get("bib", [])
                    if item["key"] == "doi"
                ]
            )
            return [
        ...
        if "identifiers" in parsed_yaml:
            return filter(
                lambda identifier: "doi" in identifier, parsed_yaml["identifiers"]
            )
    ...
    def extract_doi_from_bioconda_yaml(parsed_yaml):
        ...
        return list(
            map(
                lambda identifier_doi: identifier_doi.split(":")[1],
                get_doi_identifiers_from_yaml(parsed_yaml),
            )
        )
