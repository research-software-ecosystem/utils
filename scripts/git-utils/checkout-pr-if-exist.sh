...
RESULT=`curl -s https://api.github.com/search/issues?q=is:pr%20state:open%20label:$1%20repo:research-software-ecosystem/content`
...
TARGET_BRANCHNAME=`jq '.head.ref' <<< $(curl -s curl -s https://api.github.com/repos/research-software-ecosystem/content/pulls/$PR_NUMBER) | sed -r 's/^"|"$//g'`
